...
#import "UIImage+pwdField.h"

@interface GHPwdField ()
...
@property (nonatomic , strong) NSArray  *bottomLinesArray;

@end

@implementation GHPwdField
...
- (NSArray *)dotsArray{
    ...
    if (_dotsArray == nil) {
        ...
        for (int i = 0 ; i < _passwordLength; i ++) {
            ...
            imageView.image = [UIImage circleAndStretchableImageWithColor:self.dotColor size:CGSizeMake(self.dotWidth, self.dotWidth)];
...
-(NSArray *)bottomLinesArray{

    if (!_bottomLinesArray) {
        
        NSMutableArray *arrM = [NSMutableArray arrayWithCapacity:self.passwordLength];
        
        for ( int i = 0 ; i < self.passwordLength; i ++ ) {
            UIView *imageV = [[UIView alloc]init];
            [arrM addObject:imageV];
            imageV.backgroundColor = [UIColor blackColor];
            [self addSubview:imageV];
            
        }
        _bottomLinesArray = arrM;
    }
    
    return _bottomLinesArray;
}
...
- (void)setPasswordLength:(NSUInteger)passwordLength{
    ...
    [self.bottomLinesArray makeObjectsPerformSelector:@selector(removeFromSuperview)];
    
    [self bottomLinesArray];
    [self refreshUI];
}
...
- (void)initial{
    ...
    _passwordLength = 6;  // 密码6位
    
    _gridLineWidth = 1.0f;// 线宽 1
    
    _gridLineColor = [UIColor purpleColor]; //线条颜色
    _dotWidth = 12.0f;   // 点大小
...
- (void)layoutSubviews{
    ...
    CGFloat gridWidth = self.width / self.passwordLength ;
    ...
    for (int i = 0 ; i < self.dotsArray.count; i ++) {
        ...
        imageView.centerX = gridWidth * 0.5 + i * gridWidth;
    }
    for (int i = 0 ; i < self.labelsArray.count; i ++) {
        UILabel *label = self.labelsArray[i];
        label.width = imageWidth;
        label.height = imageHeight;
        label.centerY = self.height * 0.5;
        label.centerX = gridWidth * 0.5 + i * gridWidth;
    }
    self.textField.frame = self.bounds;
    self.backgroundImageView.frame = self.bounds;
    
    
    CGFloat unitWidth = (self.width / self.passwordLength);
    CGFloat bottomLineHeight = 3;
    CGFloat bottomLineWidht = unitWidth - 10;
    for (int i = 0 ; i < self.passwordLength; i ++) {
        UIImageView *imgV = self.bottomLinesArray[i];
        
      CGRect frame =  CGRectMake(unitWidth * i + 5 , self.height - bottomLineHeight, bottomLineWidht, bottomLineHeight);
        imgV.frame = frame;
    }
}
...
- (void)textChange:(UITextField *)textField {
    ...
    if (text.length > _passwordLength) {
        ...
        if (textField.text.length < self.passwordLength) {
            if ([self.delegate respondsToSelector:@selector(pwdField:didChangeInputPwd:)]) {
                [self.delegate pwdField:self didChangeInputPwd:self.password];
            }
        }
        else{
            if ([self.delegate respondsToSelector:@selector(pwdField:didEndInputPwd:)]) {
                [self.delegate pwdField:self didEndInputPwd:self.password];
            }
        }
...
+ (instancetype)pwdWithLength:(NSInteger)passwordLength{
    GHPwdField *passwordInputView = [[self alloc] init];
...
- (void)refreshUI{
    ...
    [self.bottomLinesArray makeObjectsPerformSelector:@selector(removeFromSuperview)];
    self.bottomLinesArray = nil;

    
    self.backgroundImageView.image = [UIImage passwordInputGridImageWithGridCount:self.passwordLength
